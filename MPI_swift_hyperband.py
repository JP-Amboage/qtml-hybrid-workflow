...
def run_then_return_validation_loss(comm, T, size, r_prev, r_i, n_next, d, k, CURVES_TABLE, MODELS, known_curve = 0.5):
	global EPOCHS 
	...
	while(True):
		...
		for worker in avail_workers:
			...
			comm.send({"trial": T[index], "iteration": 0, "epochs": ceil((r_i-r_prev)*known_curve)}, dest=worker)
		...
		for _ in all_workers:
			...
			if message["iteration"] == 1:
				...
				if n_sent_to_fully_train < k * n_next or n_sent_to_fully_train - n_fully_trained < d - len(CURVES_TABLE[r_i]):
					# fully train the configuration
					#print("CONTINUE config with id {} to worker {}".format(t['id'], source)) 
					comm.send({"trial": t, "iteration": 1, "epochs": r_i-len(t['curve'])}, dest=source)
	...
	if not r_prev+ceil((r_i-r_prev)*known_curve) in MODELS[r_i] and len(CURVES_TABLE[r_i]) >= d:
		MODELS[r_i][r_prev+ceil((r_i-r_prev)*known_curve)] = Predictor_class()
		MODELS[r_i][r_prev+ceil((r_i-r_prev)*known_curve)].train(CURVES_TABLE[r_i], n_features = r_prev+ceil((r_i-r_prev)*known_curve))
	...
	for i in range(len(PARTIALLY_TRAINED)):
		prediction = MODELS[r_i][r_prev+ceil((r_i-r_prev)*known_curve)].predict(PARTIALLY_TRAINED[i]['curve'])
	...
	while(True):
		...
		for worker in avail_workers:
			...
			comm.send({"trial": selected[index], "iteration": 1, "epochs": r_i - len(selected[index]['curve'])}, dest=worker)
...
def swift_hyperband(
		...
		for s in range(s_max, -1, -1):
			...
			for i in range(0, s+1, 1):
				...
				L, T =  run_then_return_validation_loss(comm=comm, T=T, size=size, r_prev=r_prev, r_i=r_i, n_next=n_next, d=d, k=k, CURVES_TABLE=D, MODELS=M, known_curve=known_curve)
...
@click.command()
@click.option('--dir_name', default="saved_models_swift", help='name of folder where models are saved')
...
@click.option('--d', default=15, help="minimum number of samples for training each performance predictor")
@click.option('--k', default=0.5, help="proportion of model to train in the round")
@click.option('--known_curve', default=0.5, help="position of the extra decision points")
def main(dir_name, seed, model_name, pred_type, save_models,
		r, eta, d, k, known_curve
	):
	R=r
	global Predictor_class
	...
	if (rank == 0):
		if pred_type == "classical":
			...
			if pred_type != "quantum":
				print("WARNING: pred_type not valid -> using QPredictor as default choice")
			from qpredictor import QPredictor
			Predictor_class = QPredictor
		
		rs = seed
		print(f"R={R}, eta={eta}, d={d}, k={k}, size={size}, rs={rs}, known_curve={known_curve}")
		global EPOCHS
		EPOCHS = 0
